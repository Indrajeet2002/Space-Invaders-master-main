...
class TimerDict(Timer):
  def __init__(self, frame_dict, start_key, start_index=0, delay=100, is_loop=True): 
    super().__init__(frame_dict[start_key], start_index=start_index, delay=delay, is_loop=is_loop)

    self.frame_dict = frame_dict
    self.start_key = start_key

  def reset(self):
    self.image_list = self.frame_dict(self.start_key)
    super().reset()

  def switch_to_key(self, key):
    if not self.has_key(key):
      return KeyError(f'key {key} is not in the TimerDict')
    self.image_list = self.frame_dict[key]
    self.start_index = 0
    self.last_time_switched = pg.time.get_ticks()

  def has_key(self, key): return key in self.frame_dict.keys()

  def keys(self): return self.frame_dict.keys()