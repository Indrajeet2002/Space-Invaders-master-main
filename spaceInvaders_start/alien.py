...
class Alien(Sprite): 
    ...
    def __init__(self, game, type, alien_number):
        ...
        self.timer_explosion = Timer(Alien.alien_explosion_images[random.randint(0, 2)], delay=300, is_loop=False)

        self.timer_normal = TimerDict(frame_dict={'normal': Alien.alien_images[type],
                                                  'explosion': Alien.alien_explosion_images[type]},
                                                  start_key='normal', delay=300)
    ...
    def hit(self):
        if not self.dying:
            self.dying = True
            # self.timer = self.timer_explosion
            # self.time.switch_to_key('this_will_fail')
            self.timer.switch_to_key('explosion')
